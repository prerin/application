...
function cmanOM_JS_init(){
	...
	for(var i = 0; i < cmanOM_Obj.length; i++){
		...
		if(cmanOM_Obj[i].style.position.toLowerCase() != 'absolute'){
			...
			// 背景用のdivの作成
			var wOyaBackDiv = document.createElement("div");
			wOyaBackDiv.setAttribute("id", "cmanOM_ID_BACK_DMY"+i);
			wOyaBackDiv.style.position		= 'relative';
			wOyaBackDiv.style.width		= cmanOM_Obj[i].offsetWidth + 'px';	// 大きさとmarginは元オブジェクトよりコピー
			wOyaBackDiv.style.height		= cmanOM_Obj[i].offsetHeight + 'px';
			wOyaBackDiv.style.marginTop		= wObjStyle.marginTop
			wOyaBackDiv.style.marginRight	= wObjStyle.marginRight
			wOyaBackDiv.style.marginBottom	= wObjStyle.marginBottom
			wOyaBackDiv.style.marginLeft	= wObjStyle.marginLeft
			wOyaBackDiv.style.borderRadius = "5px"
			if(cmanOM_Obj[i].tagName.toLowerCase() == 'img'){
				wOyaBackDiv.style.display = 'inline-block';
			}
			
			var wParentDiv = cmanOM_Obj[i].parentNode;
			wParentDiv.insertBefore(wOyaBackDiv,  cmanOM_Obj[i]);
			cmanOM_OyaBackObj.push(wOyaBackDiv)
		}
...
function cmanOM_JS_mdown(e){
	...
	cmanOM_VAR['moveOn']	= false;
	for(let i=0; i<cmanOM_OyaBackObj.length; i++) {
		var backobj = document.querySelector(`#cmanOM_ID_BACK_DMY${i}`)
...
function cmanOM_JS_mout(e){
	...
	cmanOM_VAR['moveOn']	= false;
	cmanOM_VAR['objNowImg'].style.top = 0
	cmanOM_VAR['objNowImg'].style.left = 0
...
function cmanOM_JS_mmove(e){
	...
	for(let i=0; i<cmanOM_OyaBackObj.length; i++) {
		var backobj = document.querySelector(`#cmanOM_ID_BACK_DMY${i}`)
		backobj.style.backgroundColor = "#dddddd"
	}
